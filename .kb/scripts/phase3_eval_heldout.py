#!/usr/bin/env python3
"""
Phase 3 — Held-out eval (READ-ONLY variant).

Identical logic to phase3_eval.py; only GOLD_PATH and output path differ.
Do NOT edit the router or index — this is a pure measurement pass on the
20 questions in gold_heldout.jsonl that were never used during tuning.
"""
...
GOLD_PATH    = KB_ROOT / 'eval' / 'gold_heldout.jsonl'   # ← HELD-OUT FILE
INDEX_DIR    = KB_ROOT / 'index'
EMBED_URL    = 'http://localhost:11434/api/embed'
EMBED_MODEL  = 'bge-m3'

# Tunable-set numbers (from eval_report_phase3.json) — used for side-by-side
TUNABLE_BASELINE = {
    'vector_retrieval@1':  45.7,
    'vector_retrieval@5':  84.8,
    'vector_retrieval@10': 89.1,
    'bm25_retrieval@1':     6.5,
    'bm25_retrieval@5':    15.2,
    'bm25_retrieval@10':   21.7,
    'routing_accuracy': '53/53',
    'exact_fact_accuracy': '100.0%',
    'localization_accuracy': '100.0%',
    'graded_count': 46,
    'total_items': 53,
}
...
def run_eval(skip_vector: bool = False) -> tuple:
    print("=" * 64)
    print("Phase 3 — Held-Out Eval (gold_heldout.jsonl, 20 Q)")
    print("=" * 64)

    con = duckdb.connect(str(DB_PATH))
    gold = load_gold()
    print(f"Loaded {len(gold)} held-out items from {GOLD_PATH}")
    ...
    for item in gold:
        ...
        # Router — unchanged, not retrained
        ...
        # Exact-fact / localization grading via DuckDB
...
def aggregate(results: list, routing_log: list) -> dict:
    ...
    routing_correct_count = sum(1 for r in routing_log if r['routing_correct'])
    ...
    # Deltas vs tunable set (not vs Phase 2)
    deltas_vs_tunable = {
        'vector_retrieval@1':  vec_r1_pct  - TUNABLE_BASELINE['vector_retrieval@1'],
        'vector_retrieval@5':  vec_r5_pct  - TUNABLE_BASELINE['vector_retrieval@5'],
        'vector_retrieval@10': vec_r10_pct - TUNABLE_BASELINE['vector_retrieval@10'],
        'bm25_retrieval@1':    bm25_r1_pct  - TUNABLE_BASELINE['bm25_retrieval@1'],
        'bm25_retrieval@5':    bm25_r5_pct  - TUNABLE_BASELINE['bm25_retrieval@5'],
        'bm25_retrieval@10':   bm25_r10_pct - TUNABLE_BASELINE['bm25_retrieval@10'],
    }

    return {
        'eval_set': 'held-out (gold_heldout.jsonl)',
        ...
        'routing_accuracy': f"{routing_correct_count}/{len(routing_log)}",
        'hallucination_rate': '0 (deterministic layer only; LLM not invoked)',
        'tunable_set_baseline': TUNABLE_BASELINE,
        'deltas_vs_tunable': {k: f"{v:+.1f}pp" for k, v in deltas_vs_tunable.items()},
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='Phase 3 held-out eval — read-only measurement')
    parser.add_argument('--skip-vector', action='store_true')
    parser.add_argument('--output', default=str(KB_ROOT / 'eval' / 'eval_report_phase3_heldout.json'))
    ...
    print()
    print("=" * 64)
    print("HELD-OUT EVAL RESULTS — Phase 3 Routed")
    print("=" * 64)
    print(f"\nGlobal metrics ({metrics['total_items']} items, {elapsed:.1f}s):")
    for k, v in metrics.items():
        if k not in ('per_domain', 'per_type', 'tunable_set_baseline', 'deltas_vs_tunable'):
            print(f"  {k}: {v}")

    print(f"\nDeltas vs tunable set (potential overfit indicator):")
    for k, v in metrics['deltas_vs_tunable'].items():
        print(f"  {k}: {v}")
    ...
    print(f"\nRouting log (all 20 questions):")
    for e in routing_log:
        mark = 'OK  ' if e['routing_correct'] else 'MISS'
        print(f"  [{mark}] [{e['id']}] gold={e['gold_domain']} routed={e['routed_domains']}")

    print(f"\nRouting errors (mis-routed):")
    errors = [r for r in routing_log if not r['routing_correct']]
    if errors:
        for e in errors:
            print(f"  [{e['id']}] gold={e['gold_domain']} routed={e['routed_domains']}")
    else:
        print("  None — all 20 questions routed correctly")
